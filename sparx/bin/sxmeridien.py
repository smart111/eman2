...
def compute_search_params(acc_trans, shifter, old_range):
	...
	return range, step
"""
def XXX_assign_particles_to_groups(minimum_group_size = 10):
	global Tracker, Blockdata
		...
		if( icone < number_of_cones ):  #  This is executed for individual number of cones, some nodes may have fewer.
			...
			for icnm in xrange(max(1,lenass)):  # I have to enter the loop even it there is no assignment
				if( lenass == 0 ):
					...
					phi,theta,psi,sx,sy, wnorm = oldparams[im][0], oldparams[im][1], oldparams[im][2], oldparams[im][3], oldparams[im][4], oldparams[im][7]
					#phi,theta,psi,sx,sy = get_params_proj(original_data[im], xform = "xform.projection")
				...
				if( lima == 1 and procid == 0):
					...
					if( lenass > 0):
						...
						for iln in xrange(keepfirst):
							...
							##junk = time()
							#xod1[iln] = -Util.sqed(data[ishift], temp, ctfa, bckgnoise)
							xod1[iln] = Util.innerproduct(data[ishift], temp, None)/nrmref  # peak replaced by CCC
							##eat += time()-junk
							##xod2[iln] = hashparams
...
def do_ctrefromsort3d_initialize(masterdir, orgstack,  option_old_refinement_dir, option_selected_cluster, option_selected_iter, shell_line_command, mpi_comm=-1):
	global Tracker, Blockdata
	if mpi_comm ==-1: mpi_comm = MPI_COMM_WORLD
	...
	if Blockdata["myid"] == Blockdata["main_node"]:
		...
		if not os.path.exists(main0_dir):
			...
			junk = cmdexecute(cmd)
		else: print(line, "%s already exits!"%main0_dir)
	if option_old_refinement_dir:
		if Blockdata["myid"] == Blockdata["main_node"]:
			line = strftime("%Y-%m-%d_%H:%M:%S", localtime()) + " =>"
			print(line, "Local refinement from previous meridien run!")
		...
		if Blockdata["myid"] == Blockdata["main_node"]:
			noiseimage        = get_im(os.path.join(old_previousoutputdir, "bckgnoise.hdf")) ## keep it
			noiseimage1       = get_im(os.path.join(old_refinement_iter_dir, "bckgnoise.hdf"))# keep it
		...
		# create directories and reassign groups
		...
		mpi_barrier(MPI_COMM_WORLD)
	else:# start from a given stack with preset header information
		if Blockdata["myid"] == Blockdata["main_node"]:
			line = strftime("%Y-%m-%d_%H:%M:%S", localtime()) + " =>"
			print(line, "Local refinement from a data stack with preset orientation parameters!")
			params = EMUtil.get_all_attributes(Tracker["constants"]["stack"], "xform.projection")
			cluster = range(len(params)
		mpi_barrier(MPI_COMM_WORLD)
	...
	if Blockdata["myid"] == Blockdata["main_node"]:
		fout = open(os.path.join(main0_dir, "Tracker_000.json"),"w")
		json.dump(Tracker, fout)
		fout.close()	
		new_chunk_one, new_chunk_two = assign_particles_to_groups(minimum_group_size = 10, asubset= cluster)
		write_text_file(new_chunk_one, os.path.join(main0_dir, "chunk_0_000.txt"))
		write_text_file(new_chunk_two, os.path.join(main0_dir, "chunk_1_000.txt"))
		write_text_file(cluster, os.path.join(main0_dir, "indexes_000.txt"))
		chunk_dict = {}
		for a in new_chunk_one: chunk_dict[a] = 0
		for b in new_chunk_two: chunk_dict[b] = 1
		# load selected iter
		new_params                     = []
		...
		for index_of_particle in xrange(len(cluster)): 
			if chunk_dict[cluster[index_of_particle]] == 0: new_params_chunk_one.append(params[cluster[index_of_particle]])
			else: new_params_chunk_two.append(params[cluster[index_of_particle]])
			new_params.append(params[cluster[index_of_particle]])
		write_text_row(new_params_chunk_one, os.path.join(main0_dir, "params-chunk_0_000.txt"))
		write_text_row(new_params_chunk_two, os.path.join(main0_dir, "params-chunk_1_000.txt"))
		write_text_row(new_params, os.path.join(main0_dir, "params_000.txt"))
	else: chunk_dict = 0
	chunk_dict = wrap_mpi_bcast(chunk_dict, Blockdata["main_node"], MPI_COMM_WORLD)
	if Blockdata["myid"] == Blockdata["main_node"]: ngroups = len(read_text_file(os.path.join(main0_dir, "groupids.txt")))
	else: ngroups = 0
	ngroups = bcast_number_to_all(ngroups, source_node = Blockdata["main_node"])
	nxx     = Tracker["constants"]["nnxo"]//2+ 1+(Tracker["constants"]["nnxo"])%2
	Blockdata["bckgnoise"] = []
	for igrp in xrange(ngroups): Blockdata["bckgnoise"].append(model_blank(nxx, 1, 1, 1.0))
	mpi_barrier(MPI_COMM_WORLD)
	## reconstruct volume using orien parameters only
	partids              = [None, None]
	partstack            = [None, None]
	particle_groups      = [None, None]
	original_data        = [None, None]
	oldparams            = [None, None]
	Tracker["directory"] = main0_dir
	if Tracker["nomsearing"]: Tracker["nxinit"] = Tracker["constants"]["nnxo"] # no smearing, start from resolution
	for procid  in xrange(2):
		partids[procid]   = os.path.join(main0_dir, "chunk_%d_000.txt"%procid)
		partstack[procid]  = os.path.join(main0_dir, "params-chunk_%d_000.txt"%procid)
		particle_groups[procid]  = os.path.join(main0_dir, "particle_groups_%01d.txt"%procid)
		original_data[procid], oldparams[procid] = getindexdata(partids[procid], partstack[procid], particle_groups[procid], original_data= None, small_memory=True, nproc =Blockdata["nproc"], myid = Blockdata["myid"])
		if Tracker["nomsearing"]: do3d_nosmearing(procid, original_data[procid], oldparams[procid])
		else:                     do3d(procid, original_data[procid], newparams, refang, rshifts, norm_per_particle, myid=Blockdata["myid"])
	do_ctrefromsort3d_get_maps_mpi(Tracker["directory"])
	if Blockdata["myid"] == Blockdata["main_node"]:
		l0143 = len(cfsc)
		cfsc = read_text_file(os.path.join(main0_dir, "driver_000.txt"))
		for icfsc in xrange(len(cfsc)):
			if cfsc[icfsc] <0.143:
				l0143 = icfsc-1
				break
		nxinit = min(Tracker["constants"]["nnxo"], l0143*2+5)
	else: nxinit = 0
	nxinit = bcast_number_to_all(nxinit, source_node = Blockdata["main_node"], mpi_comm = mpi_comm)
	Tracker["nxinit"]
	mpi_barrier(MPI_COMM_WORLD)
	mpi_finalize()
	exit()
	#compute_sigma(original_data[0]+original_data[1], oldparams[0]+oldparams[1], 0, dryrun = False, myid = Blockdata["myid"])
	return
...
def recons3d_trl_struct_MPI_nosmearing(myid, main_node, projlist, parameters, CTF, upweighted, mpi_comm, target_size):
	global Tracker, Blockdata
...
def reset_state_variables(key_list = None):
	global Tracker, Blockdata
	if not key_list: key_list = ["avgvaradj", "acc_rot", "acc_trans", "anger", "shifter", "currentres", "nxstep", "best", \
		"bestres", "no_improvement", "no_params_changes", "pixercutoff", "large_at_Nyquist", "previousoutputdir", "refvol", \
			"mainiteration", "fsc143", "accumulatepw", "maxit"
	for idex_of_key in xrange(len(key_list)):
		if key_list[idex_of_key] == "avgvaradj":         Tracker["avgvaradj"]              = [1.0, 1.0]
		if key_list[idex_of_key] == "acc_trans":         Tracker["acc_rot"]                = 0.0
		if key_list[idex_of_key] == "acc_trans":         Tracker["acc_trans"]              = 0.0
		if key_list[idex_of_key] == "anger":             Tracker["anger"]                  = 0.0
		if key_list[idex_of_key] == "shifter":           Tracker["shifter"]                = 0.0
		if key_list[idex_of_key] == "currentres":        Tracker["currentres"]             = 0
		if key_list[idex_of_key] == "nxstep":            Tracker["nxstep"]                 = 0
		if key_list[idex_of_key] == "best":              Tracker["best"]                   = 0
		if key_list[idex_of_key] == "bestres":           Tracker["bestres"]                = 0
		if key_list[idex_of_key] == "no_improvement":    Tracker["no_improvement"]         = 0
		if key_list[idex_of_key] == "no_params_changes": Tracker["no_params_changes"]      = 0
		if key_list[idex_of_key] == "pixercutoff":       Tracker["pixercutoff"]            = 0
		if key_list[idex_of_key] == "large_at_Nyquist":  Tracker["large_at_Nyquist"]       = False
		if key_list[idex_of_key] == "previousoutputdir": Tracker["previousoutputdir"]      = None
		if key_list[idex_of_key] == "refvol":            Tracker["refvol"]                 = None
		if key_list[idex_of_key] == "mainiteration":     Tracker["mainiteration"]          = 0
		if key_list[idex_of_key] == "fsc143":            Tracker["fsc143"]                 = 0
		if key_list[idex_of_key] == "accumulatepw":      Blockdata["accumulatepw"]         = [[],[]]
		if if(Blockdata["myid"] == 0): print_dict(Tracker,"state variables after resetting")
	return
...
def do_ctrefromsort3d_get_maps_mpi(ctrefromsort3d_iter_dir, regularize = True, mpi_comm = -1):
	global Tracker, Blockdata
	from mpi import MPI_COMM_WORLD, mpi_barrier
	if mpi_comm ==-1: mpi_comm = MPI_COMM_WORLD
	...
	if( Blockdata["color"] == Blockdata["node_volume"][1]):
		if( Blockdata["myid_on_node"] == 0 ):
			...
			tweight0 	= model_blank(1)
			treg0 		= model_blank(1)
		tvol0 = steptwo_mpi(tvol0, tweight0, treg0, cfsc, regularize, color = Blockdata["node_volume"][1])
		del tweight0, treg0
		if( Blockdata["myid_on_node"] == 0 ):
		...
		if( Blockdata["myid_on_node"] == 0 ):
			...
			tweight1 	= model_blank(1)
			treg1 		= model_blank(1)
		tvol1 = steptwo_mpi(tvol1, tweight1, treg1, cfsc, regularize,  color = Blockdata["node_volume"][0])
		del tweight1, treg1
		if( Blockdata["myid_on_node"] == 0 ):
