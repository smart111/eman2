...
def ali3D_primary_polar(refang, shifts, coarse_angles, coarse_shifts, procid, original_data = None, oldparams = None, \
	...
	shrink = float(Tracker["nxinit"])/float(Tracker["constants"]["nnxo"])
	radius = int(Tracker["constants"]["radius"] * shrinkage + 0.5)
	print("radius", Tracker["constants"]["radius"], radius, shrinkage, shrink, Tracker["constants"]["nnxo"], Tracker["nxinit"])
	mode = "F"
	numr = Numrinit_local(1, radius, 1, mode)
	print("numr", len(numr))
	wr = ringwe(numr, mode)
...
def do_ctrefromsort3d_get_subset_data_new(masterdir, option_old_refinement_dir, option_selected_cluster, option_selected_iter, shell_line_command):
	global Tracker, Blockdata
...
def do_ctrefromsort3d_get_subset_data(masterdir, option_old_refinement_dir, option_selected_cluster, option_selected_iter, shell_line_command):
	...
	if Blockdata["myid"] == Blockdata["main_node"]:# some numbers and path are required to be modified
		...
		update_tracker(shell_line_command) # the updated could be any refinement parameters that user wish to make change
		error_angles, error_shifts = 0.0, 0.0
		#error_angles, error_shifts        = params_changes((new_params_chunk_one + new_params_chunk_two), (new_params_chunk_one_last_iter + new_params_chunk_two_last_iter))
		# varibles in Tracker to be updated
...
def do_ctrefromsort3d_get_maps_mpi(ctrefromsort3d_iter_dir):
	...
	if Blockdata["no_of_groups"] >1: 
		if( Blockdata["myid"] == Blockdata["nodes"][1] ): 
			...
			shrank0.write_image(os.path.join(Tracker["directory"] ,"svol_0_%03d.hdf"%(Tracker["mainiteration"])))
			shrank1.write_image(os.path.join(Tracker["directory"] ,"svol_1_%03d.hdf"%(Tracker["mainiteration"])))
			del shrank0, shrank1
...
def ctrefromsorting_rec3d_faked_iter(masterdir, selected_iter=-1, comm = -1):
	...
	norm_per_particle = [[],[]]
	partstack1 = [None, None]
	for procid in xrange(2):
		if procid ==0: original_data[1] = None	
		partids[procid]   = os.path.join(Tracker["directory"],"chunk_%01d_%03d.txt"%(procid,Tracker["mainiteration"]))
		partstack[procid] = os.path.join(Tracker["constants"]["masterdir"],"main%03d"%(Tracker["mainiteration"]-1),"params-chunk_%01d_%03d.txt"%(procid,(Tracker["mainiteration"]-1)))
		partstack1[procid] = os.path.join(Tracker["constants"]["masterdir"],"main%03d"%(Tracker["mainiteration"]),"params-chunk_%01d_%03d.txt"%(procid,(Tracker["mainiteration"])))
		###
		nproc_previous = 0
		if Blockdata["myid"] == Blockdata["main_node"]:
		...
		### read_norm
		if(Blockdata["myid"] == Blockdata["main_node"]):
			junk= read_text_file(partstack1[procid], -1)
			junk = junk[7]
		else: junk=0
		junk = wrap_mpi_bcast(junk, Blockdata["main_node"], MPI_COMM_WORLD)
		norm_per_particle[procid] = junk[im_start:im_end]
		if(Blockdata["myid"] == Blockdata["main_node"]):
			print(Tracker["avgvaradj"][procid])
			for inorm in xrange(len(norm_per_particle[procid])):
				print(norm_per_particle[procid][inorm])
				info(projdata[procid][inorm])
		do3d(procid, projdata[procid], oldparamstructure[procid], refang, rshifts, norm_per_particle[procid], myid = Blockdata["myid"], mpi_comm = comm)
		projdata[procid]          = []
		oldparamstructure[procid] = []
		norm_per_particle[procid] = []
...
def do3d_nosmearing(procid, data, params, refang, rshifts, myid, mpi_comm = -1):
	global Tracker, Blockdata
...
def main():
	...
	if not options.ctrefromsort3d:
		...
		ctrefromsorting_rec3d_faked_iter(masterdir,  options.ctrefromiter)
		#### primary local 
		Blockdata["symclass"] = symclass(Tracker["constants"]["symmetry"])
		iter_local = 0
		Tracker["mainiteration"] +=1
		if(Blockdata["myid"] == Blockdata["main_node"]):
			print("AAAAAAA", Tracker["directory"], Tracker["mainiteration"])
		Tracker["avgvaradj"] = [1.,1.]
		Tracker["an"] = 6.0
		Tracker["constants"]["an"] = 6.0
		partstack         = [None, None]
		if(Blockdata["myid"] == Blockdata["main_node"]): # reset norm
			for procid in xrange(2):
				partstack[procid] = os.path.join(Tracker["constants"]["masterdir"],"main%03d"%(Tracker["mainiteration"]-1),"params-chunk_%01d_%03d.txt"%(procid,(Tracker["mainiteration"]-1)))
				junk = read_text_file(	partstack[procid], -1)
				junk[7]=[1.0]*len(junk[7])
				write_text_file(junk, partstack[procid])
		mpi_barrier(MPI_COMM_WORLD)	
		while iter_local <15:
			...
			if(Blockdata["myid"] == Blockdata["main_node"]):
				if os.path.exists(Tracker["directory"]):print(Tracker["directory"], " exists")
				else: 
			...
			mpi_barrier(MPI_COMM_WORLD)
			#refang, shifts, coarse_angles, coarse_shifts = get_refangs_and_shifts()
			...
			Tracker["refvol"] = os.path.join(Tracker["constants"]["masterdir"],"main%03d"%(Tracker["mainiteration"]-1), "vol_0_%03d.hdf"%(Tracker["mainiteration"]-1))
			for procid in xrange(2):
				partids[procid] = os.path.join(Tracker["previousoutputdir"],"chunk_%01d_%03d.txt"%(procid,(Tracker["mainiteration"]-1)))
				if(Blockdata["myid"] == Blockdata["main_node"]):
					partids[procid] = read_text_file(partids[procid])
					write_text_row(partids[procid],  os.path.join(Tracker["directory"],"chunk_%01d_%03d.txt"%(procid,Tracker["mainiteration"])))
				...
				#if(Blockdata["myid"] == Blockdata["main_node"]):
				#	partids[procid] = read_text_file(os.path.join(Tracker["directory"],"chunk_%01d_%03d.txt"%(procid,Tracker["mainiteration"])))
				#	write_text_row(partids[procid],  os.path.join(Tracker["directory"],"chunk_%01d_%03d.txt"%(procid,Tracker["mainiteration"])))
			mpi_barrier(MPI_COMM_WORLD)
			for procid in xrange(2):
				newparamstructure[procid], norm_per_particle[procid] = ali3D_primary_local_polar(refang, rshifts, coarse_angles, coarse_shifts, procid, original_data[procid], oldparams[procid], \
						preshift = True, apply_mask = True, nonorm = False, applyctf = True)
				if(Blockdata["myid"] == Blockdata["main_node"]):
					if os.path.exists( os.path.join(Tracker["directory"], "oldparamstructure")):print("oldparamsstructure exists")
					else: 
				...
				mpi_barrier(MPI_COMM_WORLD)
			if(Blockdata["myid"] == Blockdata["main_node"]):
				fout = open(os.path.join(Tracker["directory"], "Tracker_%03d.json"%Tracker["mainiteration"]),"w")
				json.dump(Tracker, fout)
				fout.close()
			ctrefromsorting_rec3d_faked_iter(masterdir, Tracker["mainiteration"])
			mpi_barrier(MPI_COMM_WORLD)
			iter_local +=1
			Tracker["mainiteration"]  +=1
			#Tracker["directory"] = os.path.join(Tracker["constants"]["masterdir"],"main%03d"%Tracker["mainiteration"])
