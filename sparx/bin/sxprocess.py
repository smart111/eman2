...
"""
	...
	if options.phase_flip:
		...
		if e1.get_zsize() == 1:  # 2D case	
			...
			for ifreq in range(len(fsc_true[0])): fsc_true[1][ifreq] = fsc_true[1][ifreq]*2./(1.+fsc_true[1][ifreq])
		...
		if nargs ==0:
			...
			from fundamentals import window2d
			from utilities import get_im
			for i in range(nimage): window2d(get_im(inputstack,i),options.box,options.box).write_image(output_stack_name,i)
